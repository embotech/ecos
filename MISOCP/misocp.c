...
void get_bounds(idxint node_idx, misocp_pwork* prob){  
    ...
    if (ret_code == ECOS_OPTIMAL){
        ...
        for (i=0; i<prob->num_int_vars; ++i){
            prob->tmp_node_id[i] = (char) round(get_node_id(node_idx,prob)[i]);
            branchable = branchable && 
                !float_eqls(get_node_id(node_idx,prob)[i], (pfloat) prob->tmp_node_id[i]);
        }
        ...
        if (prob->nodes[node_idx].U < prob->global_U){
            for(i=0; i<prob->ecos_prob->n; ++i){ prob->best_x[i] = prob->ecos_prob->best_x[i]; }
        }
