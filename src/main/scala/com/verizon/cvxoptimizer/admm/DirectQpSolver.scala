...
class DirectQpSolver(nGram: Int,
  ...
  Aeq: Option[DoubleMatrix] = None, alpha: Double = 0.0, rho: Double = 0.0, addEqualityToGram: Boolean = false) {
  ...
  val wsH = if (addEqualityToGram && linearEquality > 0) {
    //Aeq is l x rank
    //H is rank x rank
    /* wsH will look like the following */
    /* [ P + rho*I, A' ]
	   * [ A        , 0  ]
	   */
    val matAeq = Aeq.get
  ...
  def solve(q: DoubleMatrix, beq: Option[DoubleMatrix]): DoubleMatrix = {
    ...
    if(!addEqualityToGram) R = Decompose.cholesky(wsH)
    ...
    if(linearEquality > 0 && beq.get.length != linearEquality)
    	  throw new IllegalArgumentException("DirectQpSolver beq vector should match the number of linear equalities")
    ...
    while (k < MAX_ITER) {
      ...
      val xlambda = if(addEqualityToGram) {
        // If the Hessian matrix has not structure then use LU Decomposition
        //TO DO : Use LDL' decomposition for efficiency
        Solve.solve(wsH, scale)
      } else {
        // x = R \ (R' \ scale)
        //Step 1 : R' * y = scale
        //Step 2 : R * x = y
      ...
      //1. projectBound
      //2. projectPos
      //3. projectEquality or projectHyperPlane based on the structure of Aeq
      //4. proxL1
      ...
      proximal match {
        ...
        case 3 => {
          if (Aeq == None) throw new IllegalArgumentException("DirectQpSolver proximal operator on equality needs Aeq")
          if (beq == None) throw new IllegalArgumentException("DirectQpSolver proximal operator on equality needs beq")
...
object DirectQpSolver {
  def main(args: Array[String]) {
    ...
    val qpSolverPos = new DirectQpSolver(n, None, None, None, 1.0, 1.0).setProximal(1)
    ...
    val qpSolverBounds = new DirectQpSolver(n, Some(lb), Some(ub), None, 1.0, 1.0).setProximal(2)
    val boundsResult = qpSolverBounds.solve(ata, atb)
    println("Bounds result check " + (boundsResult.subi(goodBounds).norm2() < 1e-4))

    val qpSolverL1 = new DirectQpSolver(problemSize, None, None, None, 1.0, 1.0).setProximal(4)
    ...
    val qpEqStart = System.currentTimeMillis()
    val (status, qpEqResult) = qpMl.solve(Hml, fml.data)
    val qpEqTime = System.currentTimeMillis() - qpEqStart
    ...
    val directQpMl = new DirectQpSolver(ml, Some(lbeq), Some(ubeq), Some(Aeq), 1.0, 1.0, true)
    directQpMl.setProximal(1)
    
    val directQpStart = System.currentTimeMillis()
    val directQpResult = directQpMl.solve(Hml, fml, Some(beq))
    val directQpTime = System.currentTimeMillis() - directQpStart
    
    println("Sum " + directQpResult.sum())
    println("directQpResult " + directQpResult)
    println("qpEqTime " + qpEqTime + " directQpTime " + directQpTime)
    
    println("qp Equality result check " + (norm((DenseVector(directQpResult.data) - DenseVector(qpEqResult)), 2) < 1e-2))
    ...
    val qpSolverMlL1 = new DirectQpSolver(25, None, None, None, 1.0, 1.0).setProximal(4)
    ...
    val qpMlL1Result = qpSolverMlL1.solve(Hl1, fl1, None) 
