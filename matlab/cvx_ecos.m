...
if isempty( shim.name ),
    ...
    if isempty( shim ),
        shim = oshim;
        shim.error = 'Could not find a ECOS installation.';
    end
else
    shim.check = @check;
    shim.solve = @solve;
...
for k = 1 : length( nonls ),
    ...
    elseif isequal( tt, 'semidefinite' ),
        error( 'ECOS does not support semidefinite programming.' );
...
[ xx, yy, info ] = cvx_run_solver( @ecos, c, G, h, dims, A, b, 'xx', 'yy', 'info', settings, 5 );
...
if info.numerr == 2,
    status = 'Failed';
    if any( K.q == 2 ),
        warning( 'CVX:ECOS', cvx_error_format( 'This solver failure may possibly be due to a known bug in the ECOS solver. Try switching to SDPT3 by inserting "cvx_solver sdpt3" into your model.', ...
            [66,75], false, '' ) );
    end
else
    if isempty( status ),
