...
)
{
    ...
    double *Y, *D, *Lx, *Ax, flops, *X = NULL, *B = NULL, *p, *eps, *S, *SP, *delta;

    /* ---------------------------------------------------------------------- */
    /* get inputs and allocate workspace */
    /* ---------------------------------------------------------------------- */

    do_chol  = (nargin >= 3) && (nargout == 2) ;
    do_solve = (nargin >= 4) && (nargout == 1) ;
    if (!(do_chol || do_solve))
    {
	mexErrMsgTxt ("Usage:\n"
	    "  [L, D] = sldl(A, P, S);\n"
        "  [L, D] = sldl(A, P, S, eps, delta);\n"
	    "  x = sldl(A, P, S, b) ;\n"
	    "The eps/delta arguments are optional, by default we use eps=1e-14 and delta=1-e7.") ;
    }
    n = mxGetM (pargin [0]) ;
    ...
    if (do_solve)
    {
	if (mxIsSparse (pargin [3]) || n != mxGetM (pargin [3])
	    || !mxIsDouble (pargin [3]) || mxIsComplex (pargin [3]))
	{
	    mexErrMsgTxt (
    ...
    /*for( k=0; k<n; k++ ) mexPrintf("S[%d]=%6.4f\n",k,(double)S[k]);*/
    
    /* get epsilon */
    if( nargin > 3 && do_chol ){ 
        eps = mxGetPr(pargin [3]); 
    } else {
        eps = mxMalloc(sizeof(double));
        *eps = 1e-14;
    }
    
    /* get delta */
    if( nargin > 4 && do_chol ){ 
        delta = mxGetPr(pargin [4]); 
    } else {
        eps = mxMalloc(sizeof(double));
        *eps = 1e-7;
    }
    ...
    SP   = mxMalloc (nn * sizeof (double)) ; /* permuted sign vector */
	p = mxGetPr (pargin [1]) ;
	for (k = 0 ; k < n ; k++)
	{
	    P [k] = p [k] - 1 ;	/* convert to 0-based */
        SP[k] = S[P[k]];  /* permute sign vector */
	} 
    }
    else
    {
